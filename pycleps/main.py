...
def github_branches(prefix, parsed_args, **kwargs):
    ...
    if not ".git" in repo:  # Is a local directory
        ...
        branches = [b.strip() if '*' not in b else b.replace('*', '').strip() for b in branches]
        return branches
...
def main():
    ...
    parser.add_argument("--branch", required=False, help="Repository branch you want to use").completer = github_branches
