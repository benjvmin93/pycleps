...
@pytest.mark.parametrize("repo_url", ["https://github.com/TeamGraphix/graphix.git", "git@github.com:TeamGraphix/graphix.git"])
...
def test_clone_repo_already_existing(mock_env):
    ...
    add_response(mock_env, {
        ...
        f"re(^{git_clone_cmd}$)": SSHCommandMock("", "", "fatal: destination path 'repo' already exists and is not an empty directory."),
    })
...
def test_clone_repo_with_branch(mock_env):
    ...
    add_response(mock_env, {
        f"re(^{mkdir_cmd}$)": SSHCommandMock("", "", ""),
        f"re(^{git_clone_cmd}$)": SSHCommandMock("", "Cloning into 'repo'...", ""),
        f"re(^{git_checkout_cmd}$)": SSHCommandMock("", f"Switched to branch '{git_branch}'", ""),
    })
